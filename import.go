...
type H Ints
type HasPhs struct { Ph []Ph `xml:"ph"` }
type Ph struct {
    P P `xml:"p"`
    H []H `xml:"h"`
}
...
type HasSharedInput struct { Input []InputShared `xml:"input"` }
type HasMaterial struct { Material string `xml:"material,attr,omitempty"` }
type HasName struct { Name string `xml:"name,attr,omitempty"` }
type HasCount struct { Count int `xml:"count,attr,omitempty"` }
type HasType struct { Type string `xml:"type,attr,omitempty"` }
type HasId struct { Id string `xml:"id,attr,omitempty"` }
type HasUrl struct { Url Uri `xml:"url,attr,omitempty"` }
type HasSid struct { Id string `xml:"sid,attr,omitempty"` }
type HasAsset struct { Asset *Asset `xml:"asset,omitempty"` }
type HasNodes struct { Node []*Node `xml:"node"` }
type HasExtra struct { Extra []Extra `xml:"extra"` }
type HasTechniqueCommon struct { TechniqueCommon TechniqueCommon `xml:"technique_common"` }
type HasTechnique struct { TechniqueCore []TechniqueCore `xml:"technique,omitempty"` }
type HasP struct { P *P `xml:"p"` }
type HasPs struct { P []P `xml:"p"` }
